...
class JsonMetrics {
  ...
  addSplunkStats(splunkStats, ts) {
    this.add(prefixStats, 'splunkStats', splunkStats, ts);
  }
...
class SplunkMetric {
  constructor(ts, metricType) {
    ...
    metric.fields = {};
    metric.fields.metric_type = metricType;
    this.metric = metric;
  }
  add(metricName, value) {
    this.metric.fields[`metric_name:${metricName}`] = value;
  }
  toString() {
    return JSON.stringify(this.metric);
  }
}
...
class SplunkMetrics {
  ...
  addCounters(counters, counterRates, ts) {
    const collection = this.initCollection(counterLabel, ts);
    for (const counter in counters) {
  ...
  addTimers(timerData, ts) {
    const collection = this.initCollection(timerLabel, ts);
    for (const timer in timerData) {
      for (const [k, v] of Object.entries(timerData[timer])) {
        if (k == 'histogram') {
          for (const m in v) {
            collection.add(`${timer}.histogram.${m}`, v[m]);
          }
        } else {
          collection.add(`${timer}.${k}`, v);
        }
      }
    }
  }
  addGauges(gauges, ts) {
    const collection = this.initCollection(gaugeLabel, ts);
    for (const gauge in gauges) {
      collection.add(gauge, gauges[gauge]);
    } 
  }
  addSets(sets, ts) {
    const collection = this.initCollection(setLabel, ts);
    for (const set in sets) {
      collection.add(set, sets[set].size());
    }
  }
  addStatsdMetrics(statsdMetrics, ts) {
    const collection = this.initCollection(prefixStats, ts);
  ...
  initCollection(metricType, ts) {
    if (!this.metrics[metricType]) {
      this.metrics[metricType] = new SplunkMetric(ts, metricType);
    }
...
function splunkInit(startup_time, config, events, logger) {
  ...
  if (useMetrics && sourcetype == 'statsd') {
    l.log('You are attempting to send metrics events with sourcetype=statsd. Splunk attempts to process these events as raw StatsD input, which will *not* behave as expected. (https://splk.it/34HZe2k)', 'WARN')
  }
